...
export default class Scrape {
  ...
  imgbb = async (file) => {
    try {
      if (!fs.existsSync(file) && !file.includes('https:')) throw new Error("File not found");
      ...
      let streamData = file.includes('https:')?img.data : fs.createReadStream(file);
  ...
  pixhost = async (file) => {
    try {
      ...
      return `https://img92.pixhost.to/images/${res.data.show_url.split("show/")[1]}`;
    } catch (error) {
      console.log(error);
    }
  }
